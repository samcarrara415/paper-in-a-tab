...
class ObjectFieldDiscoverer implements FieldDiscoverer<Map<Field, Object>> {
    ...
    static final ObjectFieldDiscoverer EMPTY_CONSTRUCTOR_INSTANCE = new ObjectFieldDiscoverer(type -> {
        try {
            ...
            // jvmdg-desugared records keep final fields but lose the implicit
            // record machinery; allocate without a constructor and let the
            // field mapper populate them.
            final Class<?> erased = erase(type.getType());
            final Class<?> sup = erased.getSuperclass();
            if (sup != null && sup.getName().equals("xyz.wagyourtail.jvmdg.j16.stub.java_base.J_L_Record")) {
                return () -> {
                    try {
                        final Field f = sun.misc.Unsafe.class.getDeclaredField("theUnsafe");
                        f.setAccessible(true);
                        return ((sun.misc.Unsafe) f.get(null)).allocateInstance(erased);
